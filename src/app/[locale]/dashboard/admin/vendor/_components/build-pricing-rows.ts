export type PricingRow = { lane: string; service: string; detail: string; priceType: string };
...
function buildPricingDetailLines(p: Record<string, unknown>): string {
  const lines: string[] = [];
  if (p.price_per_container != null) {
    lines.push(`Per kontainer: ${formatIdrAmount(p.price_per_container)}`);
  }
  if (p.price_per_kg != null) {
    lines.push(`Per kg: ${formatIdrAmount(p.price_per_kg)}`);
  }
  if (p.price_per_cbm != null) {
    lines.push(`Per CBM: ${formatIdrAmount(p.price_per_cbm)}`);
  }
  if (p.minimum_charge != null) {
    lines.push(`Minimum charge: ${formatIdrAmount(p.minimum_charge)}`);
  }
